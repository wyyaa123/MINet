...
class MobileViTBlockv2(nn.Module):
    ...
    Args:
        ...
        ffn_mult (int): Expand the input dimensions by this factor in FFN. Default is 2.
    ...
    def __init__(
        ...
        in_channels: int,
        transformer_dim: int,
        ffn_mult: Optional[Union[Sequence[Union[int, float]], int, float]] = 2.0,
        ...
        **kwargs
        ...
        conv_1x1_in = ConvLayer2d(
            ...
            stride=1,
            use_norm=True,
            norm_layer=nn.BatchNorm2d(cnn_out_dim)
        )
...
class Reformer_v3(nn.Module):
    def __init__(self, 
        ...
        middle_attn_chans: Optional[int] = 0,
        enc_blk_nums: Optional[List[int]] = [1, 1, 1, 28],
        enc_attn_chans: Optional[List[int]] = [0, 0, 0, 0],
        dec_blk_nums: Optional[List[int]] = [1, 1, 1, 1],
        dec_attn_chans: Optional[List[int]] = [0, 0, 0, 0],
        dw_expand: Optional[int] = 2,
        ffn_expand: Optional[int] = 1,
        bias: Optional[bool] = False, 
    ):
        super(Reformer_v3, self).__init__()
        ...
        chan = width
        for num, enc_attn_chan in zip(enc_blk_nums, enc_attn_chans):
            self.downs.append(
                DownSample(chan, bias=bias)
            )
            if enc_attn_chan != 0:
                self.encoders.append(MobileViTBlockv2(in_channels=chan, transformer_dim=enc_attn_chan,
                                                      ffn_mult=ffn_expand, n_attn_blocks=num,
                                                      attn_dropout=0., dropout=.1, ffn_dropout=0.,
                                                      patch_h=patch_h, patch_w=patch_w, conv_ksize=3, dilation=1,))
                # self.encoders.append(MobileViTBlock(in_channels=chan, transformer_dim=enc_attn_chan,
                #                                     ffn_dim=enc_attn_chan * ffn_expand, n_transformer_blocks=num, head_dim=4, 
                #                                     attn_dropout=0., dropout=0., ffn_dropout=0.,
                #                                     patch_h=patch_h, patch_w=patch_w, conv_ksize=3, no_fusion=False))
            else:
        ...
        middle_blks = []
        if middle_attn_chans != 0:
            middle_blks.append(MobileViTBlockv2(in_channels=chan, transformer_dim=middle_attn_chans,
                                   ffn_dim=ffn_expand, n_attn_blocks=middle_blk_num,
                                   attn_dropout=0., dropout=.1, ffn_dropout=0.,
                                   patch_h=patch_h, patch_w=patch_w, conv_ksize=3, dilation=1,))
            # middle_blks.append(MobileViTBlock(in_channels=chan, transformer_dim=middle_attn_chans,
            #                                   ffn_dim=middle_attn_chans * ffn_expand, n_transformer_blocks=middle_blk_num, head_dim=4, 
            #                                   attn_dropout=0., dropout=0., ffn_dropout=0.,
            #                                   patch_h=patch_h, patch_w=patch_w, conv_ksize=3, no_fusion=False))
            
        else:
            middle_blks = [InvertedResidualSE(chan, expand_ratio=dw_expand, ffn_ratio=ffn_expand) for _ in range(middle_blk_num)]

        self.middle_blks = nn.Sequential(*middle_blks)

        for num, dec_attn_chan in zip(dec_blk_nums, dec_attn_chans):
            self.ups.append(
                nn.Sequential(
            ...
            self.reduce_blks.append(nn.Conv2d(chan * 2, chan, kernel_size=1, bias=bias))

            if dec_attn_chan != 0:
                self.decoders.append(MobileViTBlockv2(in_channels=chan, transformer_dim=dec_attn_chan,
                                                      ffn_dim=ffn_expand, n_attn_blocks=num,
                                                      attn_dropout=0., dropout=.1, ffn_dropout=0.,
                                                      patch_h=patch_h, patch_w=patch_w, conv_ksize=3, dilation=1,))
                # self.decoders.append(MobileViTBlock(in_channels=chan, transformer_dim=dec_attn_chan,
                #                                     ffn_dim=dec_attn_chan * ffn_expand, n_transformer_blocks=num, head_dim=4, 
                #                                     attn_dropout=0., dropout=0., ffn_dropout=0.,
                #                                     patch_h=patch_h, patch_w=patch_w, conv_ksize=3, no_fusion=False))
            else:
...
@ARCH_REGISTRY.register()
class ReformerLocal(Local_Base, Reformer_v3):
    def __init__(self, *args, train_size=(1, 3, 256, 256), fast_imp=False, **kwargs):
        Local_Base.__init__(self)
        Reformer_v3.__init__(self, *args, **kwargs)
...
if __name__ == "__main__":
    ...
    net = Reformer_v3(inp_ch=3, width=16, patch_h=2, patch_w=2, 
                      middle_blk_num=4, middle_attn_chans=256, 
                      enc_blk_nums=[2, 4, 2, 3], enc_attn_chans=[0, 0, 0, 128],
                      dec_blk_nums=[3, 2, 4, 2], dec_attn_chans=[128, 0, 0, 0], 
